...
class TestFileScanner:
    ...
    def test_ignores_virtualenv_and_cache_dirs(self, tmp_path):
        from file_scanner import FileScanner

        venv_dir = tmp_path / ".venv" / "Lib" / "site-packages"
        cache_dir = tmp_path / "__pycache__"
        valid_dir = tmp_path / "infra"
        venv_dir.mkdir(parents=True)
        cache_dir.mkdir(parents=True)
        valid_dir.mkdir()

        (venv_dir / "ignored.yaml").write_text("key: from-venv")
        (cache_dir / "ignored.json").write_text('{"key": "from-cache"}')
        (valid_dir / "main.tf").write_text('resource "aws_s3_bucket" "b" {}')

        found = list(FileScanner().scan(str(tmp_path)))
        found_names = {path.name for path in found}

        assert "main.tf" in found_names
        assert "ignored.yaml" not in found_names
        assert "ignored.json" not in found_names
...
class TestAnalyzerPromptFormatting:
    def test_iam_prompt_includes_rule_severity_suffix(self):
        from misconfigguard.analysis.iam_analyzer import _build_iam_prompt

        prompt = _build_iam_prompt(
            assignments=[],
            query="Check IAM risks",
            security_references=[
                {"title": "Least privilege", "severity": "HIGH", "text": "Avoid broad RBAC."}
            ],
        )

        assert "### [Rule 1] Least privilege (HIGH)" in prompt

    def test_secrets_prompt_includes_rule_severity_suffix(self):
        from misconfigguard.analysis.secrets_analyzer import _build_secrets_prompt

        prompt = _build_secrets_prompt(
            findings=[],
            query="Check hardcoded secrets",
            security_references=[
                {"title": "Secrets rule", "severity": "MEDIUM", "text": "Use a secret store."}
            ],
        )

        assert "### [Rule 1] Secrets rule (MEDIUM)" in prompt

    def test_workload_identity_prompt_includes_rule_severity_suffix(self):
        from misconfigguard.analysis.workload_identity_analyzer import _build_workload_identity_prompt

        prompt = _build_workload_identity_prompt(
            configs=[],
            query="Check workload identity trust",
            security_references=[
                {"title": "OIDC rule", "severity": "LOW", "text": "Pin issuer and subject."}
            ],
        )

        assert "### [Rule 1] OIDC rule (LOW)" in prompt
